...
| 单个字符串字面量上限 64 KiB | 见下 |
| 反向读标识符上限 512 字符 | 包里的 base64 块不会被当成变量名整段拷出来 |

跨度上限同时解决时间问题：失锚的开括号原先会被拿去和几十 MB 外的闭括号配对，让每个坏站点都付出一次全包扫描。

字符串上限是**第二处**无界分配，第一轮修复漏掉了：正则字面量或模板里的引号看起来就是字符串开头，而配对的引号可能在几 MB 之外、甚至根本不存在。`readStringLiteral` 逐字符累加，一次失锚就把整个 46 MB 的包读成一个字符串——在每个试探锚点上都来一次，单这一条就足以耗尽堆。

探测锚点的调用方用 `tryReadStringLiteral`（返回 `null`），真正解析已确认字面量的地方用会抛错的版本。两者的区别是语义上的：前者"这里不是锚点"是正常结果，后者"读不下去"是错误。
